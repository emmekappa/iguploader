...
export class InstagramClient {
  ...
  private readonly userCookiePath = path.join(
    os.tmpdir(),
    ".igUploaderState.json"
  );
  ...
  async login(force = false): Promise<void> {
    ...
    try {
      ...
      if (!currentUser.username) throw new Error("not logged in");
      else console.log("State still looks valid");
    } catch (error) {
