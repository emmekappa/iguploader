...
export const IgDropzone: FunctionComponent<DropzoneAreaProps> = (
  props: PropsWithChildren<DropzoneAreaProps>
) => {
  ...
  const onAdd = async (addedFileObjects: FileObject[]): Promise<void> => {
    ...
    for (const fileObject of addedFileObjects) {
      const validationResult = await instagramIpcInvoker.validatePhoto({
        filePath: fileObject.file.path,
      });
      if (!validationResult.isValid) {
        enqueueSnackbar(
          `${fileObject.file.name} invalid: ${validationResult.reason}`,
          {
            variant: "error",
            persist: true,
          }
